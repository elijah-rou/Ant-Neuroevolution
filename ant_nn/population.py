...
class Population:
    ...
    def __init__(self, popSize, mutationRate, mutationStrength, \
        keepThresh, numInputs, numOutputs, layerSizes):
        ...
        self.mutationStrength = mutationStrength # variance of gaussian mutation function (needs testing)
        ...
        self.chromosomes = self.initializePop(numInputs, numOutputs, layerSizes) # list of weights
    ...
    def makeChromosome(self, numInputs, numOutputs, hiddenSizes):
        ...
        for i in range(len(hiddenSizes)+1):
            if i == 0: # first layer
                chromosome += [randomnessWidth * (np.random.rand(numInputs, hiddenSizes[0]) \
                    - (0.5-randomnessCenter))]
            elif i == len(hiddenSizes): # last layer
                chromosome += [randomnessWidth * (np.random.rand(hiddenSizes[-1], numOutputs) \
                    - (0.5-randomnessCenter))]
            else: # middle layers
                chromosome += [randomnessWidth * (np.random.rand(hiddenSizes[i-1], hiddenSizes[i]) \
                    - (0.5-randomnessCenter))]
        ...
        for i in range(len(chromosome)):
            ...
            for j in range(chromosome[i].shape[0]):
                for k in range(chromosome[i].shape[1]):
                    if random.random() < self.mutationRate: # only mutate a gene w some small prob
